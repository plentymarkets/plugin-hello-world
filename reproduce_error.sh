...
for i in {1..30}; do
  ...
  for n in {1..5}; do
    ...
    sleep 0.5  # ⬅️ întârziere mică, ca să se calce unul pe altul
  done
  ...
  echo "$RESPONSE" | jq '{startTime: .startTime, finishTime: .finishTime, stepsStatus: .stepsStatus, buildErrors: .buildErrors}'

  if echo "$RESPONSE" | grep -q "worktree contains unstaged changes"; then
    echo "🎯 Reprodus la iterația $i!"
    break
  fi
